...
export function ClipBaseDialog({
  ...
  const isComposite = useMemo(() => {
    ...
    return (
      !!(segments && segments.length > 0) || clip.type === ClipType.COMPOSITE
    );
  }, [clip]);
  ...
  useEffect(() => {
    ...
    if (isComposite) {
      ...
      if (dur < MIN_CLIP_DURATION) {
        setValidationError(
          `Effective duration must be at least ${MIN_CLIP_DURATION}s`
        );
        return;
      }
    } else {
            ...
            <DialogTitle className="flex items-center gap-2">
              {mode === 'view' ? (
                ...
                <>
                  ...
                  {isComposite ? 'Fine-Tune Segments' : 'Edit Time Range'}
                </>
              )}
            </DialogTitle>
              ...
              {src ? (
                <VideoPlayerUI
                  ...
                  endTime={
                    mode === 'edit'
                      ? isComposite
                        ? editSegments[editSegments.length - 1]?.end ||
                          media?.duration ||
                          0
                        : editEnd
                      : clip.end
