...
> {
  ...
  private async performUpload(
    ...
    // Hold a temp-dir lease for the whole download+upload window so a
    // sibling step (e.g. speaker transcription) finishing first can't delete
    // the shared per-media temp directory out from under this transfer.
    return this.storageService.withTempLease(input.mediaId, async () => {
      try {
        ...
        // deferred automatically while a sibling step still holds a temp
        // lease (the last holder performs the deletion).
