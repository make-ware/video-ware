...
async function downloadPocketBase() {
  try {
    ...
    if (fs.existsSync(executablePath)) {
      ...
      try {
        const version = execSync(
          `cd "${pbDir}" && ./${executableName} --version`,
          { encoding: 'utf8' }
        );
    ...
    if (needsDownload) {
      console.log(
        `📦 Setting up PocketBase v${POCKETBASE_VERSION} for ${platform}/${arch}...`
      );
      ...
      if (process.platform === 'win32') {
        ...
        execSync(
          `powershell -command "Expand-Archive -Path '${zipPath}' -DestinationPath '${pbDir}' -Force"`,
          { stdio: 'inherit' }
        );
      } else {
        // Use unzip on Unix-like systems
        execSync(`cd "${pbDir}" && unzip -o "${filename}"`, {
          stdio: 'inherit',
        });
      }
