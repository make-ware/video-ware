...
migrate((app) => {
  ...
  const collection_TimelineRenders_add_processor_1 = app.findCollectionByNameOrId("TimelineRenders");

  collection_TimelineRenders_add_processor_1.fields.add(new TextField({
    name: "processor",
    required: false
  }));

  app.save(collection_TimelineRenders_add_processor_1);
  ...
  app.save(collection_TimelineRenders_revert_add_version);

  const collection_TimelineRenders_revert_add_processor = app.findCollectionByNameOrId("TimelineRenders");

  collection_TimelineRenders_revert_add_processor.fields.removeByName("processor");

  return app.save(collection_TimelineRenders_revert_add_processor);
});
