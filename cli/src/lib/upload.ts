...
/**
 * Extensions the ingest pipeline understands. The worker detects the media
 * type (video/audio/image) from the extension, so an unknown one would be
 * misclassified as video rather than rejected server-side.
 */
      ...
      for (const chunk of chunks) {
        ...
        for (let attempt = 0; ; attempt++) {
          try {
            ...
            // `complete` on the last chunk, or early when a retried request
            // finds the record already finalized (idempotent route response).
            if (result.complete) {
            ...
            const retryable =
              !(err instanceof ChunkRequestError) || err.retryable;
