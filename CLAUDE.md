...
- **Track keyframe reads** (`shared/src/utils/keyframes.ts`, `shared/src/mutators/label-track.ts`, `cli/src/lib/track.ts`): `LabelTrack.keyframes` is the heaviest column in the database — capped at 10 MB per row, one box per ~0.125s — and one media routinely carries hundreds of tracks, so **never read `LabelTrack` without a `fields` projection unless you actually need per-frame geometry**. Two projections are sanctioned: `TRACK_SUMMARY_FIELDS` (every column but `keyframes`, plus the four `expand.*` paths, since naming any field strips expands) and `TRACK_KEYFRAME_FIELDS` (`id,keyframes`), and a drift test fails the build when a new column joins neither. Storage contract: keyframe `t` is **absolute media seconds** (`keyframes[0].t === track.start`) and boxes are **0–1 fractions of the frame** — parse through `normalizeKeyframes`, which doubles as the rebaser (`normalizeKeyframes(raw, track.start)`) rather than having two functions. Invariants: `boundingBox` is the union of the frames, written by the four spatial normalizers via `unionBbox`, healed onto hash-matched rows by the step processors, and backfilled by `1785740000_labeltrack_boundingbox_backfill.js` — **never bump a processor version to re-emit it**, since `trackHash` includes the version and a bump duplicates every track in the library; readers still derive the union when the column is empty, so correctness never depends on the backfill having run. Bulk keyframe reads (`vw track export`, `vw workspace export --tracks`) fetch **one track at a time**, estimate their output from `trackData.frameCount` **before** the first keyframe request so an over-budget ask costs nothing, and write to a file — a terminal is not a data sink. Speech and speaker tracks store `keyframes: []` by design; an empty list there is a valid track, not a missing one.
