...
vw frame                       # write one still JPEG from a media or media clip (from the filmstrip previews)
...
`-c` takes an offset into what the clip plays, not a source-media time — the
same domain as `vw timeline clips map --offset`, so a composite clip's cut
gaps are skipped. The JSON always names the domain it used
(`at: { domain, value }`).

There is no `--timeline-clip`. A timeline clip is either a window onto a media
— in which case its `MediaRef`/`MediaClipRef` is what you want, and
`vw timeline clips show <id>` gives you both — or a nested timeline, whose
frames are a composite only the renderer can produce.

```bash
# what does this clip look like 3s in?
vw frame -c MEDIACLIP_ID --at 3 -o /tmp/shot.jpg

# from a timeline clip: read its source, then frame that
vw timeline clips show CLIP_ID --json     # -> .clip.MediaClipRef / .clip.MediaRef
vw timeline clips map CLIP_ID --timeline-time 42 --json   # -> .point.source
...
- `frame` → `{ source: { kind, id }, mediaId, at: { domain, value },
  ...
  media seconds (`"media"`, from `-m`) or as a clip offset (`"offset"`, from
  `-c`). `warnings` also goes to stderr
