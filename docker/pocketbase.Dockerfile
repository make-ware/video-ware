...
# Install curl (healthcheck / diagnostics)
...
# Create the /data tree and own it as nextjs:nodejs (uid 1001). Named volumes
# inherit this ownership; bind mounts must be chowned to 1001 on the host
# (see docker/README.md "Data directory & permissions").
RUN mkdir -p /data/pb_data /data/storage && \
    chown -R nextjs:nodejs /data

# Entrypoint seeds/updates the superuser, then serves. It runs as nextjs (uid
# 1001) — see USER below — so the superuser upsert and serve both create/own the
# database files as 1001, which avoids the "attempt to write a readonly
